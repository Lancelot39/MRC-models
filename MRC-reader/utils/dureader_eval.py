...
def compute_bleu_rouge(pred_dict, ref_dict, bleu_order=4):
    ...
    bleu_model=Bleu(bleu_order)
    for cand,ref in zip([i for i in pred_dict.values()],[j for j in ref_dict.values()]):
        bleu_model.add_inst(cand[0],ref)
    bleu_scores= bleu_model.compute_score()
    ...
    rouge_model=Rouge()
    for cand,ref in zip([i for i in pred_dict.values()],[j for j in ref_dict.values()]):
        rouge_model.add_inst(cand[0],ref)
    rouge_score=rouge_model.compute_score()
