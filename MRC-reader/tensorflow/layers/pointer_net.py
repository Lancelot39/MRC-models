...
class MRCDecoder():
    ...
    def decode(self,p,q):
        with tf.variable_scope('MRC-decoder'):
            hq=tf.contrib.layers.fully_connected(q,num_outputs=1,activation_fn=None)
            s=tf.matmul(tf.transpose(tf.nn.softmax(hq,1),[0,2,1]),q)
            sm=tf.tile(s,[1,tf.shape(p)[1],1])
            fusion=tf.contrib.layers.fully_connected(tf.concat([p,sm,p*sm,p-sm],-1),num_outputs=self.hidden_size,activation_fn=tf.tanh)
            p1=tf.nn.softmax(tf.contrib.layers.fully_connected(fusion,num_outputs=1,activation_fn=None),1)
            
            l=tf.matmul(tf.transpose(p1,[0,2,1]),p)
        with tf.variable_scope('MRC-fusion'):
            fusion_v=tf.concat([s,l,s*l,s-l],-1)
            x_hat=tf.contrib.layers.fully_connected(fusion_v,num_outputs=self.hidden_size,activation_fn=tf.nn.relu)
            g=tf.contrib.layers.fully_connected(fusion_v,num_outputs=self.hidden_size,activation_fn=tf.sigmoid)
            o=g*x_hat+(1-g)*s

        with tf.variable_scope('MRC-end'):
            om=tf.tile(o,[1,tf.shape(p)[1],1])
            
            fusion_end=tf.contrib.layers.fully_connected(tf.concat([p,om,p*om,p-om],-1),num_outputs=self.hidden_size,activation_fn=tf.tanh)
            p2=tf.nn.softmax(tf.contrib.layers.fully_connected(fusion_end,num_outputs=1,activation_fn=None),1)
        return tf.squeeze(p1,-1),tf.squeeze(p2,-1)
