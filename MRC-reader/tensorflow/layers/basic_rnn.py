...
def get_cell(rnn_type, hidden_size, layer_num=1, dropout_keep_prob=None):
    ...
    return cells
def highway_network(input):
    hidden_size=input.get_shape().as_list()[-1]
    s=tf.contrib.layers.fully_connected(input,num_outputs=hidden_size,activation_fn=tf.nn.relu)
    g=tf.contrib.layers.fully_connected(input,num_outputs=hidden_size,activation_fn=tf.sigmoid,biases_initializer=tf.constant_initializer(-2.0))
    return g*s+(1-g)*input
